...
HRESULT D3DRenderer::CompileShaderFromFile(const WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, const D3D_SHADER_MACRO* defines, ID3DBlob** ppBlobOut)
{
	...
	ID3DBlob* pErrorBlob;
	hr = D3DCompileFromFile(szFileName, defines, mpIncludeDir, szEntryPoint, szShaderModel,
...
void D3DRenderer::setTextureResource(int index, Texture2D* texture)
{
	if (mpActiveShader)
	{
...
Texture2D* D3DRenderer::CreateTexture(D3D11_TEXTURE2D_DESC* textureDescription, DXGI_FORMAT resViewFmt)
{
	Texture2D* newTexture = NE_NEW Texture2D();
...
void D3DRenderer::RenderDeferredShading()
{
	mpd3dImmediateContext->OMSetRenderTargets(1, &mpRenderTarget, mpDepthStencilView);
